...
\setlength\LTleft{-0.7in} % set left and right margins for longtables, to center without considering document margins
...
% Lengths of indent and space for new paragraphs
...
\psfig{file=Images/Misc/logopm,width=4cm}
...
\vspace{2.0cm}
\begin{Large}
Android Lifecycle and Event-Based Testing
\end{Large}  
\end{center}
...
\part*{Abstract}
Mobile market is continuously expanding, and mobile applications require higher and higher quality standards. The only way a developer can obtain confidence of correctness and usability in a set of specific circumstances is testing.
...
In addition to lifecycle transitions, mobile applications are characterized by a great number of other events, hundreds of which may be processed every minute, such as sensor data, connectivity changes, user input, network responses, etc. These events can happen in many different orders and frequencies, and, if the developer does not handle them correctly, issues may arise. The aim of the second part of this thesis is to describe an event-based testing approach that allows to observe several events during the application execution and to express consistency checks on their stream by means of a temporal assertions language.

\emptypage


\part*{Estratto}
Il mercato dei dispositivi mobili \`{e} in continua espansione, e le applicazioni  richiedono standard di qualit\`{a} sempre pi\`{u} alti. L'unico modo per ottenere una sufficiente confidenza che l'applicazione si comporti nel modo desiderato in determinate situazioni \`{e} il testing.
...
The lifecycle of a Fragment is closely related to the lifecycle of the Activity that contains it: for example, when an Activity is paused, all the contained Fragments are paused too. In addition to this, however, Fragments can go through lifecycle changes independently of their host Activity: more in detail, since Fragments can be dynamically added and removed at runtime, they can be created and destroyed while the Activity is running. Moreover, the developer also has the option to store removed Fragments in the so-called backstack and be able to restore them later for example when the user presses the "back" button.
...
The work presented in this thesis address some of the limitations of the testing frameworks and tools just presented. In particular, it provides a better support for lifecycle testing from both a static and a dynamic point of view, and a comprehensive approach to test events in Android applications, focusing not only on race conditions but also on ordering, existence and causality.
...
This section presents static lifecycle checks in depth. First a short introduction of the main concepts of static program analysis is presented, then a detailed explanation of its application in lifecycle testing.
...
\begin{itemize}
	\item InTheClear~\cite{InTheClear} is an application for alerting during emergencies. Before commit \textit{cabfc26}\footnote{\url{https://github.com/SaferMobile/InTheClear/commit/cabfc2643bc5820e109433a8ae48f9f6a0788d9f}} a Broadcast Receiver was registered but never unregistered in the \texttt{SMSSender} class. Using the static lifecycle tests the developers would have detected the issue immediately, without the need to receive the log notification and open the issue \#56\footnote{\url{https://github.com/SaferMobile/InTheClear/issues/56}}.
...
\subsubsection{Real-World Application} \fix{And mention if developers have added any new test cases. (?)}
...
The solution proposed in the second part of this thesis aims at being a complementary tool to these technique. Instead of focusing on automatic detection, it gives the means to manually specify event-related conditions during standard unit, integration or UI tests. This means that this approach presents the advantage of avoiding the issue of false positives and false negatives, intrinsic for detection tools, but the developer has to code test cases by hand to cover the relevant parts of the application.

More in depth, with the proposed event-based testing library the developer is able to specify which events to observe during the execution of the application and then to define consistency checks on their stream, such as happens-before relationship, ordering and existence.

This approach allows to detect race conditions like the other tools, but the scope is not limited to data races: the developer is able to specify any type of condition on any event generated by the application, from user input to sensor data callbacks. Moreover, the tool does not only apply to race conditions but it also allows to focus on ordering among similar events, to quantify the amount of events of a certain type during an execution and so on.
...
indexOf(e, m, i) \equiv \exists_{=i} e' \Big( match(e', m) \land before(e', e) \Big)
  ...
  \rotatebox[origin=c]{90}{\textbf{ Visual }} & 
	...
	\raisebox{-240pt}{\includegraphics[width=400px]{Images/Event/Lang/Slide7.PNG}}
...
ordered(com, e1, e2) \equiv \bigg( \Big( com<0 \Rightarrow before(e1, e2) \Big) \lor\\ \Big( com=0 \Rightarrow before(e1, e2) \lor before(e2, e1) \Big) \lor\\ \Big( com>0 \Rightarrow before(e2, e1) \Big) \bigg)
  ...
  \rotatebox[origin=c]{90}{\textbf{ Visual }} & 
	...
	\raisebox{-170pt}{\includegraphics[width=400px]{Images/Event/Lang/Slide8.PNG}}
...
\begin{longtable}{ | m{0.3cm} | m{15cm} | }
  ...
  \rotatebox[origin=c]{90}{\textbf{ Visual }} & 
	...
	\raisebox{-170pt}{\includegraphics[width=390px]{Images/Event/Lang/Slide9.PNG}}
...
\begin{longtable}{ | m{0.3cm} | m{15cm} | }
  ...
  \rotatebox[origin=c]{90}{\textbf{ Visual }} & 
	...
	\raisebox{-200pt}{\includegraphics[width=390px]{Images/Event/Lang/Slide16.PNG}}
...
\begin{longtable}{ | m{0.3cm} | m{15cm} | }
  ...
  \rotatebox[origin=c]{90}{\textbf{ Visual }} & 
	...
	\raisebox{-200pt}{\includegraphics[width=390px]{Images/Event/Lang/Slide17.PNG}}
...
\begin{longtable}{ | m{0.3cm} | m{15cm} | }
  ...
  \rotatebox[origin=c]{90}{\textbf{ Visual }} & 
	...
	\raisebox{-200pt}{\includegraphics[width=390px]{Images/Event/Lang/Slide18.PNG}}
...
\begin{longtable}{ | m{0.3cm} | m{15cm} | }
  ...
  \rotatebox[origin=c]{90}{\textbf{ Visual }} & 
	...
	\raisebox{-105pt}{\includegraphics[width=390px]{Images/Event/Lang/Slide11.png}}
...
\begin{longtable}{ | m{0.3cm} | m{15cm} | }
  ...
  \rotatebox[origin=c]{90}{\textbf{ Visual }} & 
	...
	\raisebox{-70pt}{\includegraphics[width=390px]{Images/Event/Lang/Slide13.PNG}}
...
\begin{itemize}
	...
	\item Settings: RxPreferences~\cite{RxPreferences} module gives the possibility to receive events from the Shared Preferences system (storage provided by Android to store the app settings).
...
As a real-world example of event-based testing application, the WordPress for Android app was chosen. The idea is to define some temporal assertions on the latest version at the time of writing~\cite{WordPress2016-07-15}, to discuss the expressiveness of the language, the generation effort and the types of events that can be observed.
