...
This part of the thesis explains how we can express these consistency checks among events by means of a temporal assertions language. The idea is to specify assertions similar to the standard JUnit-like way, i.e. statements that either succeed or fail and that throw an \texttt{AssertionError} in the latter case, but with a validity that spans the whole execution instead of the single instant in which they are invoked. A detailed explanation of the language to express checks is reported in the next sections.
...
\forall e1 \bigg( match(e1, m1) \Rightarrow \exists e2 \Big( match(e2, m2) \land before(e2, e1) \Big) \bigg)
  ...
  \rotatebox[origin=c]{90}{\textbf{ Visual }} & 
	...
	Note in particular that in the second case the check succeeds even if no blue event occurs: we are  just saying that blue events \textit{can} happen after an orange one.
...
\begin{longtable}{ | m{0.3cm} | m{15cm} | }
  ...
  \rotatebox[origin=c]{90}{\textbf{ Description }} & 
  	...
  	Checks that \textit{all} the events that match \texttt{m} match in order \texttt{m$_1$}, \texttt{m$_2$},..., \texttt{m$_n$}.
...
\begin{longtable}{ | m{0.3cm} | m{15cm} | }
  ...
  \rotatebox[origin=c]{90}{\textbf{ Description }} & 
  	...
  	All sub-checks \texttt{c1}, \texttt{c2}, etc. must succeed.
  ...
  \rotatebox[origin=c]{90}{\textbf{ Code Example }} & 
	...
	allHold(
		...
		exactly(1).eventsWhereEach(isServiceStart(mainService))
...
The generation effort to build components used by the system depends on the specific application and on what the developer wants to achieve: here a general discussion is reported, followed by a more concrete report on a real-world application in section \ref{event_eval_real}.
